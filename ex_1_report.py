...
def classify_point_to_centroid_index(all_points, current_centroids):
    ...
    current_points_centroid_dis = []
    # for each point
    for point in all_points:
        d = list()
        # calculate distance for point to each centroid
        for current_centroid in current_centroids:
            d.append(distance_between_two_points(point[0], point[1], current_centroid[0], current_centroid[1]))
        # append the index of the nearset centroid
        current_points_centroid.append(d.index(min(d)))
        current_points_centroid_dis.append(min(d))
    return current_points_centroid,current_points_centroid_dis;
...
distances = []
f = open("output.txt", "w+")
while iterations < 10 and not np.array_equal(centroids, old_centroids):
    # classify each point to centroid
    points_classifications, points_classifications_dis = classify_point_to_centroid_index(points, centroids)
    # update centroids
    ...
    for centroid_index in range(0, len(centroids)):
        ...
        if count_points_classify > 0:
            ...
            centroids[centroid_index]=(x_mean,y_mean)
        centroid_index += 1
    print(f"[iter {iterations}]:{','.join([str(j) for j in centroids])}")
    f.write(f"[iter {iterations}]:{','.join([str(j) for j in centroids])}")
    f.write("\n")
    iterations += 1
    distances.append(sum(points_classifications_dis)/len(points_classifications_dis))
'''for pdf'''
